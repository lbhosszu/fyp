...
# Optional: keep your existing FastF1 summary feature
import fastf1
...
# -----------------------------
# Sidebar UI (offline from dataset)
# -----------------------------
st.sidebar.header("Race Selection (from dataset)")
...
with colA:
    if st.button(f"Predict Top{k}"):
        ...
        st.subheader(f"Predicted Top{k} for {gp_name} {season}")

        # Score: how many true top-k are in our predicted top-k
        score = int(preds.head(k)["TrueTopK"].sum())
        st.success(f"Score: {score}/{k} correct drivers in the Top{k}")

        # Show table (predicted top-k first)
        st.dataframe(preds, use_container_width=True)

with colB:
    st.markdown("### Race summary (FastF1)")
    st.caption("Optional. Uses FastF1 live calls, which may fail on your current network/Python setup.")

    if st.button("Load race summary (FastF1)"):
        try:
            with st.spinner("Loading race data..."):
                results, weather = get_race_summary(season, gp_name)

            st.subheader(f"{gp_name} {season} summary")
            st.dataframe(results, use_container_width=True)

            st.subheader("Weather summary")
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Avg air temp (°C)", f"{weather['avg_air_temp']:.1f}")
            c2.metric("Avg track temp (°C)", f"{weather['avg_track_temp']:.1f}")
            c3.metric("Max wind (m/s)", f"{weather['max_wind']:.1f}")
            c4.metric("Rain", "Yes" if weather["rain_any"] else "No")
        except Exception as e:
            st.error(f"FastF1 summary failed: {e}")

st.info("Pick a season + GP + mode, then click Predict. The model loads instantly from models/ and data/.")
